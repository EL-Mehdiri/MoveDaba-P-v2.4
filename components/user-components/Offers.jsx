...
const Offers = ({ user }) => {
  ...
  // Function to handle navigation to broken items page with query params
  const handleBrokenItemsNavigation = (offet) => {
    if (offer) {
      const brokenItemsUrl = `/broken-items?orderId=${offer.purchaseOrder}`;
      router.push(brokenItemsUrl);
    }
  };
      ...
      <Modal open={open} onClose={() => setOpen(false)}>
        {offer && (
          <div className="flex flex-col gap-4 overflow-auto h-[500px]">
            ...
            border border-red-400 rounded-md text-red-800  text-xl
                      bg-white hover:bg-gray-50 shadow-2xl hover:text-gray-600"
            ...
            <p className="text-lg font-semibold">
              Prix:{" "}
              <span className="font-normal">{Math.floor(offer?.price)}DH</span>
            </p>
            ...
            <div className="flex justify-center">
              ...
              <button
                onClick={() => handleBrokenItemsNavigation(offer)}
                className=" rounded-full py-2 text-sm px-10 bg-[#94D726] hover:bg-lime-300 text-white ml-4"
              >
                Biens endommagés
              </button>
            </div>
